...
To use h2o models introduced in MB-pol(2023), set the monomer name to "h2o\_2x3y4z", where x, y, z each is replaced by one of the three characters "a", "b", or "c". Please refer to the paper for more details on what "a", "b", and "c" correspond to. In addition, unless z="a", you also need to set "fourbody\_cutoff=6.5" in the MBX input json file. If z="a", "fourbody\_cutoff" does not affect the model, but should be set to 0 for better performance.
- MB-pol
